...
def receive_file(dir_list, socket_connected_count, con, addr_ip, addr):
    ...
    try:
        ...
        print(file_to_send.decode(), "file Size Is:", file_size >> 20, "MB /", os.path.getsize(file_to_send),
              "KB")
        ...
        f = open(log_path, "a")
        f.write(
            dt_string + " " + file_to_send + ' Has Been Transferred Successfully To ' + addr_ip + " File Size: " + str(
                file_size) + ' KB\n')
        f.close()
        ...
        f = open(log_path, "a")
        f.write(
            dt_string + " Bad File Request from " + addr_ip + " File " + file_to_send.decode() + " Not Found On Server\n")
        f.close()
        socket_connected_count += 1

    return socket_connected_count


def send_file(socket_connected_count, con, addr_ip):
    try:
        ...
        f = open(log_path, "a")
        f.write(dt_string + " File " + str(
            file_sent_name) + " Has Been Successfully Received From " + addr_ip + " File Size: " + str(
            file_sent_size) + " KB\n")
        f.close()
...
def quit_app(con, addr_ip, dt_string):
...
def print_connectivity(socket_connected_count, addr, addr_ip):
...
def main_app(socket_connected_count):
    while True:
        ...
        if socket_connected_count == 0:
            print_connectivity(socket_connected_count, addr, addr_ip)

        send_recv = con.recv(1024)
        send_recv = send_recv.decode()

        if send_recv == "r":
            receive_file(dir_list, socket_connected_count, con, addr_ip, addr)

        if send_recv == 's':
            send_file(socket_connected_count, con, addr_ip)

        if send_recv == 'q':
            quit_app(con, addr_ip, dt_string)

#        return addr_ip, socket_connected_count
...
main_app(socket_connected_count)
